#!/usr/bin/env bash
set -Eeuo pipefail
trap 'rc=$?; echo "P4A_BOOTSTRAP_FAILED rc=$rc line=$LINENO"; exit "$rc"' ERR
...
[[ -s "$SNAPSHOT" ]] || { echo P4A_T0_SNAPSHOT_NOT_FOUND; exit 2; }
...
cp "$SOLVER_SRC/hpc/p4a_clean_start.slurm" "$RUN_DIR/run.slurm"
...
echo "P4A_JOB_ID=${JOB_ID%%;*}"
echo "P4A_RUN_DIR=$RUN_DIR"
echo P4A_BOOTSTRAP_COMPLETE
