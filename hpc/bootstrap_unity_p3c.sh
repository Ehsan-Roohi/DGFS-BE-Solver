#!/usr/bin/env bash
# One-line Unity bootstrap for the Phase 3C t=30 -> 31 decay audit.
set -Eeuo pipefail
trap 'rc=$?; echo "P3C_BOOTSTRAP_FAILED rc=$rc line=$LINENO"; exit "$rc"' ERR
...
if [[ -n "${DGFS_SNAPSHOT:-}" ]]; then
  ...
  SNAPSHOT=$({ find "$DGFS_ROOT/runs" -maxdepth 2 -type f -name 'dist_dgfs_fig14b-30.0.frfss' -printf '%T@ %p\n' 2>/dev/null || true; } | sort -nr | head -1 | cut -d' ' -f2-)
fi
[[ -s "$SNAPSHOT" ]] || { echo P3C_SNAPSHOT_NOT_FOUND; exit 2; }
...
cp "$SOLVER_SRC/hpc/p3c_restarts.slurm" "$RUN_DIR/run.slurm"
printf 'source_snapshot=%s\nsource_ref=%s\n' "$SNAPSHOT" "$DGFS_REF" > "$RUN_DIR/INPUT.txt"

JOB_ID=$(cd "$RUN_DIR" && sbatch --parsable \
  --export=ALL,DGFS_ROOT="$DGFS_ROOT",DGFS_ENV="$DGFS_ENV",DGFS_SOLVER_SRC="$SOLVER_SRC" ./run.slurm)
echo "P3C_JOB_ID=${JOB_ID%%;*}"
echo "P3C_RUN_DIR=$RUN_DIR"
echo P3C_BOOTSTRAP_COMPLETE
